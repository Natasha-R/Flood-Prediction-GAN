...
def create_path(save, model, data_path, split, dataset_subset, dataset_dem, not_input_topography, resize, crop, epoch):
    ...
    path = f"{data_path}/data/{save}s/{model}_topography{not not_input_topography}_{split}_{dataset_subset}_{dataset_dem}DEM_resize{resize}_crop{crop}_epoch{epoch}_{str(datetime.now())[:-7].replace(' ', '-').replace(':', '-')}{file_type}"
    return path

def initialise_loss_storage(model, overall):
    pre_string = ""
    if overall:
        pre_string = "all_"
...
def print_losses(model, epoch, all_losses):
